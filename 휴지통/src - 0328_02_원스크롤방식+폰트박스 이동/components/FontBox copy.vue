<template>
  ...
  <div :class="device === 'mobile' ? 'block md:hidden': 'hidden md:block' " class="z-50">
    <div :class="isOpen && 'top-50' || device === 'mobile' && 'top-0'" class="darkMode text-center  cursor-pointer transition-all duration-500">
      <font-awesome-icon @click="!isOpen ? isOpen = true : isOpen = false" icon="pencil" class="text-2xl"/>
    </div>
    <ul :class="isOpen && '!top-20' || (device === 'mobile' & isOpen) && 'mt-[700px]'" class="absolute w-52 -top-44 bg-white darkMode trasition-all duration-500">
