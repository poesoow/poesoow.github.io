    ...
    <div class="max-w-7xl mx-auto mt-8">
      ...
      <div v-for="e in CateItem" :key="e" class="bg-white mb-8 pt-12 px-8 pb-16 rounded-md flex border flex-wrap darkMode group">
        ...
        <div class="basis-full md:basis-[52%] pt-10  group-even:order-2 xl:group-even:order-1">
          <h3 class="text-2xl font-bold py-[10px] md:pl-[150px] lg:pl-[50px]"> {{ e.descTitle }}</h3>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">{{ e.desc }}</p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">키워드 : <span v-for="el in e.keyword" :key="el" class="mr-2">{{ el }}</span></p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">컬러 : <span v-for="el in e.color" :key="el" :class="el === '' && 'hidden'" class="w-max h-5 inline-block align-middle mr-2 text-sm text-center" :style="{backgroundColor: `${el}`}">{{ el }}</span></p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">폰트 : {{ e.font }}</p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">사용툴 : <span v-for="el in e.tools" :key="el" class="mr-2 align-middle">{{ el }}</span></p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">작업기간 : {{ e.date }}</p>
          <p class="text-base mt-[10px] md:pl-[150px] lg:pl-[50px]">기여도 : {{ e.contribution }}</p>
          <ul class=" flex justify-center mt-6">
            <li :class="e.progress === '' && 'hidden'"><a :href="e.progress" class="py-1 px-5 border rounded-md darkMode text-sm">progress</a></li>
            <li v-if="e.original" class="mx-3"><a :href="e.original" target="_blank" class="py-1 px-5 border rounded-md darkMode text-sm">original</a></li>
            <li v-if="e.original" ><a :href="e.redesign" target="_blank" class="py-1 px-5 border rounded-md darkMode text-sm">redesign</a></li>
          </ul>
        </div>
      </div>
    </div>
