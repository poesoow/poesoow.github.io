<template>
  ...
  <div :class="device === 'mobile' ? 'block md:hidden': 'hidden md:block' " class="z-50">
    <div :class="isOpen && 'top-50' || device === 'mobile' && 'top-0'" class="text-center  cursor-pointer">
      <font-awesome-icon @click="!isOpen ? isOpen = true : isOpen = false" icon="pencil" class="text-2xl"/>
    </div>
    <ul :class="isOpen && '!right-8' || device === 'mobile' && 'mt-[700px]'" class="absolute w-40 -right-44 bg-white darkMode trasition-all duration-500">
      <li @click="FontSelect(index)" @mouseleave="FontRemove(index)" @mouseover="FontView(index)" v-for="(e, index) in FontList" :key="e" class="cursor-pointer text-left py-2" :class="e.font">{{ e.title }}</li>
    </ul>
  </div>
...
<script>
  export default {
    ...
    props:{
      device: String,
      // 0330 폰트박스 실습때는 부자관계라서 한번만 props 해서 가능 나는 한단계 더 있어서 아직 성공 못함
      langList: Object
    },
    data() {
      return {
